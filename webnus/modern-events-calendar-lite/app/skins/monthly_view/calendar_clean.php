    ...
    <?php
        ...
        for($list_day = 1; $list_day <= $days_in_month; $list_day++)
        {
            ...
            if(isset($events[$today]) and count($events[$today]))
            {
                ...
                $events_str .= '<div class="mec-calendar-events-sec" data-mec-cell="'.$day_id.'" '.(trim($selected_day) != '' ? ' style="display: block;"' : '').'><h6 class="mec-table-side-title">'.sprintf(__('Schedule for %s', 'modern-events-calendar-lite'), date_i18n('F', $time)).'</h6><h3 class="mec-color mec-table-side-day"> '. $date_suffix .'</h3>';
